...
En esta variante, el pivote se selecciona como el último elemento del arreglo. Aunque es fácil de implementar, esta estrategia puede llevar a un rendimiento deficiente en casos donde los datos ya están ordenados o casi ordenados. La complejidad promedio esperada es \(O(n \log n)\), pero el peor caso es \(O(n^2)\) cuando el pivote no divide el arreglo de manera equilibrada.

\subsubsection{Pivote primer elemento}
En esta variante, el pivote se selecciona como el primer elemento del arreglo. Similar a la selección del último elemento, esta estrategia también puede resultar en un rendimiento deficiente para datos ordenados o casi ordenados. La complejidad promedio esperada es \(O(n \log n)\), pero el peor caso es \(O(n^2)\) debido a la falta de equilibrio en la partición.
...
La complejidad temporal de la búsqueda binaria es \(O(\log n)\) en todos los casos (mejor, promedio y peor), debido a que siempre divide el espacio de búsqueda por la mitad en cada iteración, independientemente del estado de los datos. La búsqueda binaria es eficiente para grandes conjuntos de datos ordenados, pero no es adecuada para datos no ordenados.
...
Su complejidad temporal es \(O(1)\) en el mejor caso cuando el elemento buscado está en la primera posición. En el caso promedio, la complejidad es \(O(\log i)\), donde \(i\) es la posición del elemento buscado, ya que la fase de expansión exponencial realiza aproximadamente \(\log i\) iteraciones y la búsqueda binaria posterior también es \(O(\log i)\). En el peor caso es \(O(\log n)\). La búsqueda exponencial es especialmente útil para arreglos de tamaño desconocido o muy grandes, donde la búsqueda binaria tradicional no sería aplicable.
...
A continuación se presenta una tabla resumen de las complejidades temporales de los algoritmos discutidos:
...
\hline
\textbf{Merge Sort Optimizado} & $O(n \log n)$ & $O(n \log n)$ & $O(n \log n)$ \\
\hline
\textbf{Quick Sort (último elemento)} & $O(n \log n)$ & $O(n \log n)$ & $O(n^2)$ \\
\hline
\textbf{Quick Sort (primer elemento)} & $O(n \log n)$ & $O(n \log n)$ & $O(n^2)$ \\
\hline
\textbf{Quick Sort (elemento aleatorio)} & $O(n \log n)$ & $O(n \log n)$ & $O(n^2)$ \\
\hline
\textbf{Quick Sort (mediana de tres)} & $O(n \log n)$ & $O(n \log n)$ & $O(n^2)$ \\
\hline
\textbf{Búsqueda Binaria Recursiva} & $O(\log n)$ & $O(\log n)$ & $O(\log n)$ \\
\hline
\textbf{Búsqueda por Interpolación} & $O(1)$ & $O(\log \log n)$ & $O(n)$ \\
\hline
\textbf{Búsqueda Exponencial} & $O(1)$ & $O(\log i)$ & $O(\log n)$ \\
\hline
\textbf{Búsqueda por Rango} & $O(\log n)$ & $O(\log n + k)$ & $O(n)$ \\
\hline
\textbf{Quick Select} & $O(n)$ & $O(n)$ & $O(n^2)$ \\
\hline
\end{tabular}
\caption{Complejidades temporales de los algoritmos de ordenamiento y búsqueda. La variable $i$ representa la posición del elemento buscado en búsqueda exponencial, y $k$ representa la cantidad de elementos coincidentes en búsqueda por rango.}
